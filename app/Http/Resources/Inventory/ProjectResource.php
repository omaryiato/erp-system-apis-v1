...
class ProjectResource extends JsonResource
{
    ...
    {
        $totalRevenue = $this->when( $this->relationLoaded('revenues'),
                fn () => (float)
                    $this->revenues->sum('amount'), 0 );

        $paidAmount = $this->when( $this->relationLoaded('cashTransactions'),
                fn () => (float) $this->cashTransactions
                ->whereIn('transaction_type',
                            [ 'income', 'other_income',
                            'revenue_payment', ]) ->sum('amount'), 0 );
